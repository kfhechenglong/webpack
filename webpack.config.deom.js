...
var config = {
    ...
    entry: {
        'vendors': [
            'jquery',
            'vue',
            __dirname + '/vendors/wangeditor/css/wangEditor.min.css',
            __dirname + '/vendors/wangeditor/js/wangEditor.js',
            __dirname + '/vendors/uuid.js'
        ]
    },
    //入口文件输出配置
    output: {
    ...
    module: {
        ...
        {
            test: __dirname + '/vendors/wangeditor/js/wangEditor.js',
            loader: 'expose?wangEditor'
        },
        {
            test: __dirname + '/vendors/uuid.js',
            loader: 'expose?uuid'
        },
        {
            test: /\.js$/,
    ...
    plugins: [
        ...
        new webpack.ProvidePlugin({
            ...
            'uuid': __dirname + '/vendors/uuid.js'
        }),
        new copyWebpackPlugin([{
            from: path.resolve(__dirname, 'vendors/uuid.js'),
            to: path.resolve(__dirname, '../web/assets/js')
        },{
             from: path.resolve(__dirname, 'vendors/wangeditor/js/emotions.data'),
            to: path.resolve(__dirname, '../web/assets/js')
        }])
    ],
    resolve: {
